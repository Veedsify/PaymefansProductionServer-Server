...
module.exports = async (body) => {
  ...
  try {
    ...
    const adminUserId = await prismaQuery.user.findFirst({
    ...
    if (!adminUserId) {
      ...
      return {
        error: "Sorry, an error occurred while creating your account",
        status: false,
      };
    }
    ...
    await CreateFollowing(adminUserId.id, user.id);
